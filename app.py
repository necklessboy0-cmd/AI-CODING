...
"""
    try:
        response = client.models.generate_content(model="gemini-3.6-flash", contents=instruction)
...
# ============================================================
# GROK / OPENAI-120 ERROR DETECTION & CORRECTION
# ============================================================
...
"""
    try:
        ...
        if not result:
            return {"success": False, "error": "Model returned an invalid validation response.", "corrected_code": "", "error_report": ""}
        return {
            "success": True,
            "error": "",
...
if error_check_button:
    ...
    if not app_code_to_check.strip():
        ...
        ebox = st.empty()
        ebox.markdown("""<div class="status-card"><div style="font-size:32px;">😂</div><div class="status-text"><strong>Validating your app.py...</strong></div></div>""", unsafe_allow_html=True)
...
st.markdown(
    """
    <div class="footer">
        AI Code Builder • Prompt → Review → Generate → Validate → Fix
    </div>
    """,
    unsafe_allow_html=True,
)
