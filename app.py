...
st.markdown(
    ...
    <div class="section-card">
        ...
        <div class="section-description">
            Record your project idea naturally. Gemini will understand
            the recording, extract the requirements and add the analyzed
            voice input to your main prompt.
        </div>
    </div>
    """,
    unsafe_allow_html=True,
...
def analyze_voice_with_gemini(audio_bytes):
    """Understand the user's spoken project request."""
...
Extract the actual project idea, explicit features, UI requirements, inputs and outputs,
behavior/workflow, technical constraints, programming preferences, and important technical
names, numbers, package names and model names. Be faithful to what the user said. Do not
invent unrelated requirements. If something is genuinely unclear, write [unclear] rather
than guessing. Do not generate application code.
...
def generate_project_with_gemini(
    ...
    """Generate the complete project with Gemini 3.5 Flash-Lite."""
    client=get_gemini_client()
    if client is None:
        return {"success": False, "error": "Please add GEMINI_API_KEY to Streamlit Secrets.", "files": {}}
    instruction=f"""
You are the senior software engineer for AI Code Builder.
Generate the complete project from the approved Master Prompt.
...
2. Implement the user's requested functionality.
3. Treat voice-derived requirements as user requirements unless marked [unclear].
4. Do not invent unrelated features.
5. Include proper error handling.
6. Never expose API keys or secrets.
7. Generate an accurate requirements.txt.
8. Generate a useful README.md.
9. Add supporting files only when genuinely required.
10. For Streamlit projects, use app.py as the main file.
11. Keep imports consistent with requirements.txt.
12. Do not leave TODO placeholders for required functionality.
13. Return ONLY valid JSON with no Markdown fences.
...
"""
    try:
        response=client.models.generate_content(model="gemini-3.5-flash-lite", contents=instruction)
