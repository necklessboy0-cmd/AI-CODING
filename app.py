...
# ============================================================
# GEMINI & GROK CONFIGURATION (Moved Up to Fix Scope Errors)
# ============================================================
...
st.markdown(
    ...
    .stApp {
        background:
            radial-gradient(circle at 10% 10%, rgba(129, 140, 248, 0.20), transparent 30%),
            radial-gradient(circle at 90% 20%, rgba(236, 72, 153, 0.16), transparent 30%),
            radial-gradient(circle at 50% 100%, rgba(45, 212, 191, 0.12), transparent 35%),
            linear-gradient(135deg, #0b1020 0%, #111827 50%, #0f172a 100%);
        color: #f8fafc;
    }
    ...
    div[data-testid="stTextArea"] textarea {
        border-radius: 16px !important; background: rgba(15, 23, 42, 0.75) !important;
        color: #f8fafc !important; border: 1px solid rgba(255, 255, 255, 0.10) !important;
    }
    div[data-testid="stTextInput"] input {
        border-radius: 14px !important; background: rgba(15, 23, 42, 0.75) !important; color: #f8fafc !important;
    }
...
def analyze_voice_with_gemini(audio_bytes):
    """Understand the user's spoken project request safely now that get_gemini_client is defined above."""
...
if analyze_voice_button and voice_audio is not None:
    voice_box = st.empty()
    voice_box.markdown(
        """<div class="status-card"><div style="font-size:32px;">😂</div><div class="status-text"><strong>Gemini is analyzing your voice...</strong><br>Extracting your project intent and requirements.</div></div>""",
        unsafe_allow_html=True,
    )
    ...
    if result["success"]:
        ...
        voice_block = ("--- VOICE INPUT ANALYZED BY GEMINI ---\n" + result["analysis"] + "\n--- END VOICE INPUT ---")
        if voice_block not in existing:
            st.session_state.prompt_text = (f"{existing}\n\n{voice_block}".strip() if existing else voice_block)
...
if st.session_state.voice_analysis:
    st.info("🎙️ Voice analysis is included in the main prompt and will be reviewed by Gemini before code generation.")
...
def review_prompt_with_gemini(user_prompt, programming_language, coding_skill, output_style, github_url=""):
    client = get_gemini_client()
    if client is None:
        return {"success": False, "error": "Gemini API key is not configured. Please add GEMINI_API_KEY to Streamlit Secrets.", "reviewed_prompt": ""}
    try:
        review_instruction = f"""
You are the Prompt Architect for an AI Code Builder. Your job is to analyze and improve the user's coding request so another AI coding model can generate the application accurately.
...
"""
        ...
        if not reviewed_prompt:
            return {"success": False, "error": "Gemini returned an empty response.", "reviewed_prompt": ""}
        return {"success": True, "error": "", "reviewed_prompt": reviewed_prompt}
    except Exception as error:
        return {"success": False, "error": f"Gemini prompt review failed: {error}", "reviewed_prompt": ""}
...
def check_and_correct_code_with_grok(app_code, requirements, master_prompt):
    client = get_grok_client()
    if client is None:
        return {"success": False, "error": "GROK_API_KEY is not configured in Streamlit Secrets.", "corrected_code": "", "error_report": ""}
    
    review_prompt = f"""
You are the final senior code-review and debugging system. Inspect the generated app.py and correct it.
...
"""
    try:
        response = client.chat.completions.create(
            ...
            temperature=0.1,
            messages=[
                {"role": "system", "content": "You are a senior Python and Streamlit debugging engineer. Return only valid JSON."},
                {"role": "user", "content": review_prompt},
            ],
        )
        result = extract_json_from_response(response.choices[0].message.content)
        if not result:
            return {"success": False, "error": "Grok returned an invalid validation response.", "corrected_code": "", "error_report": ""}
        return {
            "success": True,
            "error": "",
            "corrected_code": result.get("corrected_code", app_code),
            "error_report": result.get("error_report", "No issues found."),
            "has_errors": result.get("has_errors", False)
        }
    except Exception as error:
        return {"success": False, "error": f"Grok validation failed: {error}", "corrected_code": "", "error_report": ""}
...
if st.session_state.generation_complete:
    ...
    error_check_button = st.button("🔎 Run Error Detection & Auto-Fix", type="primary", use_container_width=True)
else:
...
if error_check_button:
    ...
    if not app_code_to_check.strip():
        ...
        ebox = st.empty()
        ebox.markdown("""<div class="status-card"><div style="font-size:32px;">😂</div><div class="status-text"><strong>Grok is validating your app.py...</strong></div></div>""", unsafe_allow_html=True)
        grok_result = check_and_correct_code_with_grok(app_code_to_check, requirements_to_check, master_prompt_to_check)
        ebox.empty()
        if grok_result["success"]:
            st.session_state.error_report = grok_result["error_report"]
            st.session_state.corrected_code = grok_result["corrected_code"]
            st.session_state.project_files["app.py"] = grok_result["corrected_code"]
            st.session_state.error_check_complete = True
            st.success("✅ Error detection and correction completed.")
        else:
            st.error(f"❌ {grok_result['error']}")
